...
if sel is None:
    ...
    # 동일 주문에서는 1회만 띄우기
    just_finished = delivered_done and (st.session_state.get("done_banner_for") != selected_id)
    if just_finished:
        st.session_state["done_banner_for"] = selected_id

    ele_path = "assets/icons/home/elephant.png"
    ele_src  = local_image_to_data_url(ele_path)

    overlay_html = ""
    if just_finished:  # 완료되자마자 1회
        overlay_html = f"""
        <style>
          .done-overlay {{
            position:absolute; inset:0;    /* 부모 박스 기준 */
          ...
          .done-card {{
            ...
            gap: 14px; min-width: 520px; max-width: 86%;
          }}
          .done-card img {{ width: 180px; height: auto; }}
          .done-title {{ color:#fff; font-size:22px; font-weight:800; letter-spacing:0.2px; }}
          .done-sub   {{ color:#fff; font-size:20px; font-weight:700; }}
        </style>
        <div class="done-overlay">
          <div class="done-card">
            {"<img src='" + ele_src + "' alt='elephant' />" if ele_src else ""}
            <div class="done-title">딜리펀트 프로토타입 - 배달 완료</div>
            <div class="done-sub">{n_min if n_min is not None else "예상"}분 만에 배달이 완료되었어요 <span style="font-size:22px">☺️</span></div>
          </div>
        </div>
        """
