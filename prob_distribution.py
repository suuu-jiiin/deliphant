...
st.set_page_config(page_title="확률분포 대시보드", layout="wide")
...
def load_data():
    """데이터를 로드하고 캐싱합니다."""
    try:
        df = pd.read_csv('prob_distribution.csv').head(10)
    except FileNotFoundError:
        st.warning("⚠️ prob_distribution.csv 파일을 찾을 수 없어 예제 데이터로 실행합니다.", icon="⚠️")
        data = {
            'ID': [f'ID_{i}' for i in range(1, 6)],
            '1.0': [0.1, 0.2, 0.05, 0.15, 0.05],
            '1.5': [0.5, 0.3, 0.15, 0.25, 0.1],
            '2.0': [0.3, 0.4, 0.10, 0.40, 0.2],
            '2.5': [0.05, 0.05, 0.20, 0.15, 0.5],
            '3.0': [0.05, 0.05, 0.50, 0.05, 0.15],
...
# --- 메인 앱 ---
st.header("📈 예상 배달소요시간 확률분포 상세보기")

# ID 선택을 위한 Selectbox
id_list = df['ID'].tolist()
selected_id = st.selectbox('확인할 ID를 선택하세요:', id_list)
